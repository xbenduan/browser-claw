...
const formatMessage = (value: string, vars?: Record<string, string | number>) => {
...
export const I18nProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const listener = (changes: Record<string, chrome.storage.StorageChange>) => {
      const prefs = changes.preferences?.newValue as { language?: Locale } | undefined;
  ...
  const messages = useMemo<Messages>(() => (translations[locale] ?? translations.zh) as Messages, [locale]);

  const t = useCallback(
    (key: string, vars?: Record<string, string | number>) => {
      const value =
        getNestedValue(messages as unknown as Record<string, any>, key) ??
        getNestedValue(translations.zh as unknown as Record<string, any>, key) ??
        key;
