...
const queryStoredDataSkill: SkillDefinition = {
  ...
  name: '查询存储数据',
  description: '通过指针 ID 查询之前 API 调用返回的大型数据。支持路径导航、数组切片（offset/limit）、字段过滤（filter）和字段投影（fields）。当你在 tool result 中看到 "[STORED DATA — pointer: ...]" 时，使用此工具获取完整数据。',
  version: '1.0.0',
  api: {
    method: 'GET',
  ...
  parameters: [
    {
      ...
      location: 'query',
      required: true,
      description: '数据指针 ID，如 "ref_getUsers_1_abc123"。在之前的 tool result 中可以找到。',
    },
    {
      name: 'path',
      type: 'string',
      location: 'query',
      required: false,
      description: '数据路径（点分格式），用于导航到嵌套数据。例如 "data.items" 或 "result.users"。不填则返回顶层数据。',
    },
    {
      name: 'offset',
      type: 'number',
      location: 'query',
      required: false,
      description: '数组切片起始索引（0-based），默认 0。与 limit 配合实现分页浏览。',
      default: 0,
    },
    {
      name: 'limit',
    ...
    },
    {
      ...
      location: 'body',
      required: false,
      description: '简单字段过滤条件。格式 { "fieldName": "value" }。字符串值支持模糊匹配（不区分大小写），其他类型精确匹配。',
    },
    {
      name: 'fields',
      type: 'array',
      location: 'body',
      required: false,
      description: '字段投影列表，只返回指定字段。例如 [["id", "name", "email"]。不填返回所有字段。',
      items: {
        type: 'string',
        description: '字段名',
      },
