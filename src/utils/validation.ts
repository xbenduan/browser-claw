...
export function validateSkill(skill: unknown): ValidationResult {
  ...
  // 基础字段校验
  if (!s.id || typeof s.id !== 'string') errors.push('Missing or invalid "id" (string)');
  if (!s.name || typeof s.name !== 'string') errors.push('Missing or invalid "name" (string)');
  ...
  if (!api || typeof api !== 'object') {
    ...
    if (api.baseUrl && typeof api.baseUrl === 'string' && !api.baseUrl.startsWith('http')) {
  ...
  if (!Array.isArray(parameters)) {
    ...
    for (let i = 0; i < parameters.length; i++) {
      ...
      if (!p.name || typeof p.name !== 'string') errors.push(`parameters[${i}]: missing "name"`);
  ...
  if (meta && typeof meta === 'object') {
    if (meta.riskLevel && !VALID_RISK_LEVELS.includes(meta.riskLevel as string)) {
      errors.push(`meta.riskLevel must be one of: ${VALID_RISK_LEVELS.join(', ')}`);
    }
  }
  ...
  if (!binding || typeof binding !== 'object') {
    ...
    if (!Array.isArray(binding.hostPatterns) || binding.hostPatterns.length === 0) {
