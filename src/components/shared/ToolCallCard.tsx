  ...
  return (
    <div className="glass-card p-3 my-2">
      <div className="flex items-center justify-between gap-2">
        ...
        <span
          className={`px-2 py-0.5 rounded-full text-[10px] border flex items-center gap-1 ${statusConfig.cls}`}
        >
      ...
      {toolCall.confirmation && (
        <div className="mt-2 bg-slate-50/90 rounded-lg p-2 border border-slate-200/70 font-mono text-[10px] flex items-center gap-2 overflow-x-auto custom-scrollbar">
          <span
            className={`font-bold shrink-0 ${METHOD_COLORS[toolCall.confirmation.method] ?? "text-slate-400"}`}
          >
            {toolCall.confirmation.method}
          </span>
          <span
            className="text-slate-600 whitespace-nowrap"
            title={toolCall.confirmation.url}
          >
        ...
        <div className="mt-3 space-y-1">
          <div className="h-1.5 w-full bg-slate-200/70 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${(toolCall.batchProgress.completed / toolCall.batchProgress.total) * 100}%`,
              }}
            />
          </div>
          <div className="flex justify-between text-[10px] text-slate-600 px-0.5">
            <span>
              {t("toolCall.progress", {
                completed: toolCall.batchProgress.completed,
                total: toolCall.batchProgress.total,
              })}
            </span>
            <div className="flex gap-2">
              <span className="text-green-600">
                {t("toolCall.successCount", {
                  count: toolCall.batchProgress.success,
                })}
              </span>
              <span className="text-red-600">
                {t("toolCall.failedCount", {
                  count: toolCall.batchProgress.failed,
                })}
              </span>
            </div>
          </div>
